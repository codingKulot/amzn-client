              ...
              <form>
                <div class="a-spacing-medium a-spacing-top-medium">
                  ...
                  <div class="a-spacing-top-medium">
                    ...
                    <input type="text" class="a-input-text" style="width: 100%;" v-model="fullName" :placeholder="address.fullName" required/>
                  </div>
                  <!-- Street Address -->
                  <div class="a-spacing-top-medium">
                  ...
                  <div class="a-spacing-top-medium">
                    ...
                    <input type="text" class="a-input-text" style="width: 100%;" v-model="city" :placeholder="address.city" required/>
                  </div>
                  <!-- State -->
                  <div class="a-spacing-top-medium">
                    <label style="margin-bottom: 0px;">State / Province / Region</label>
                    <input type="text" class="a-input-text" style="width: 100%;" v-model="state" :placeholder="address.state" required/>
                  </div>
                  <!-- Zip Code -->
                  <div class="a-spacing-top-medium">
                    <label style="margin-bottom: 0px;">Zip Code</label>
                    <input type="text" class="a-input-text" style="width: 100%;" v-model="zipCode" :placeholder="address.zipCode" required/>
                  </div>
                  <!-- Phone Number -->
                  <div class="a-spacing-top-medium">
...
export default {
    ...
    async asyncData({ $axios, params }) {
        try {
            // let countries = $axios.$get('http://localhost:3000/api/countries');
            // let singleAddress = $axios.$get(`http://localhost:3000/api/user-address/${params.id}`);

            let countries = $axios.$get('/api/countries');
            let singleAddress = $axios.$get(`/api/user-address/${params.id}`);
            
            let [countriesResponse, addressResponse] = await Promise.all([
                countries, singleAddress
            ]);

            return {
                countries: countriesResponse,
                address: addressResponse.userAddress
            };
        } catch (err) {
            console.log(err);
        }
    ...
    methods: {
        async onUpdateAddress() {
            ...
            // let response = await this.$axios.$put(`http://localhost:3000/api/user-address/${this.$route.params.id}`, data);
            let response = await this.$axios.$put(`/api/user-address/${this.$route.params.id}`, data);
