                  ...
                  <div class="col-lg-4 col-md-4 col-sm-12 pl-md-0 pb-2" v-for="(address, index) in addresses" :key="address._id">
                    <div class="a-box a-spacing-none normal-desktop-address-tile">
                      ...
                      <div class="edit-address-desktop-link">
                        <nuxt-link :to="`/user-address/${address._id}`">Edit</nuxt-link>
                        &nbsp; | &nbsp;
                        <!-- Delete Button -->
...
export default {
  async asyncData({ $axios }) {
    try {
      // let response = await $axios.$get('http://localhost:3000/api/user-addresses');
  ...
  methods: {
    async onDeleteAddress(id, index) {
      try {
        // let response = await this.$axios.$delete(`http://localhost:3000/api/user-addresses/${id}`);
        let response = await this.$axios.$delete(`/api/user-addresses/${id}`);
    ...
    async onSetDefaultAddress(id) {
      try {
        // let response = await this.$axios.$put(`http://localhost:3000/api/user-addresses/set/default`, {id: id});
        let response = await this.$axios.$put(`/api/user-addresses/set/default`, {id: id});
