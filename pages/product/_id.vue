                    ...
                    <div class="col-sm-5 col-5">
                      ...
                      <!-- <span>Qty:
                        <select @change="onChangeQuantity($event, product)">
                          <option v-for="i in 25" :key="i" :value="i" :selected="checkQty(product.quantity, i)">&nbsp;{{ i }}</option>
                        </select>
                      </span> -->
                    </div>
                  </div>
                    ...
                    <span class="a-spacing-small a-button-primary a-button-icon">
                      <span class="a-button-inner">
                        ...
                        <span class="a-button-text">
                          <nuxt-link to="/place-order" class="a-button-text">Buy Now</nuxt-link>
                        </span>
                      </span>
                    </span>
                  </div>
...
export default {
  ...
  async asyncData({ $axios, params }) {
    try {
      // let singleProduct = await $axios.$get(`http://localhost:3000/api/product/${params.id}`);
      // let manyReviews = await $axios.$get(`http://localhost:3000/api/review/${params.id}`);

      let singleProduct = await $axios.$get(`/api/product/${params.id}`);
      let manyReviews = await $axios.$get(`/api/review/${params.id}`);
      ...
      return {
        ...
        reviews: reviewResponse.productReview
      }
    } catch (err) {
      console.log(err);
    }
